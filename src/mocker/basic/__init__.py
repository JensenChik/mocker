...
def generate_string(charset, length):
    return ''.join([choice(charset) for _ in range(length)])


def int_between_1_and(max_len):
...
def String(count=1, selection=None, charset=ALPHABET, fixed_len=None, max_len=None):
    ...
    if fixed_len is not None:
        if isinstance(fixed_len, int):
            return [generate_string(charset, fixed_len) for _ in range(count)]
        elif isinstance(fixed_len, list):
            assert len(fixed_len) == count, 'if fixed_len is list, len must equal to count'
            return [generate_string(charset, _fixed_len) for _fixed_len in fixed_len]
        else:
            raise Exception('fixed_len support int or list only')

    if max_len is not None:
        if isinstance(max_len, int):
            return [generate_string(charset, int_between_1_and(max_len)) for _ in range(count)]
        elif isinstance(max_len, list):
            assert len(max_len) == count, 'if max_len is list, len(max_len) must equal to count'
            return [generate_string(charset, int_between_1_and(_max_len)) for _max_len in max_len]
        else:
